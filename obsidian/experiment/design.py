    ...
    def initialize(
        ...
        method_dict = {
            "LHS": lambda d, m: torch.tensor(
                qmc.LatinHypercube(
                    d=d, scramble=False, seed=seed, strength=1, optimization="random-cd"
                ).random(n=m)
            ),
            "Random": lambda d, m: torch.rand(size=(m, d)),
            "Sobol": lambda d, m: draw_sobol_samples(
                bounds=torch.tensor([0.0, 1.0]).reshape(2, 1).repeat(1, d), n=m, q=1
            ).squeeze(1),
            "Custom": lambda d, m: torch.tensor(sample_custom),
            "DOE_full": lambda d, m: torch.tensor(
                factorial_DOE(d=d, n_CP=3, shuffle=True, seed=seed, full=True)
            ),
            "DOE_res4": lambda d, m: torch.tensor(
                factorial_DOE(d=d, n_CP=3, shuffle=True, seed=seed)
            ),
        }
