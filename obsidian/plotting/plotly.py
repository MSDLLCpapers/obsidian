...
def visualize_inputs(campaign: Campaign) -> Figure:
    ...
    if "X_best_f_idx" in dir(campaign.optimizer):
        marker_shapes = [
            "diamond" if rowInd in [campaign.optimizer.X_best_f_idx] else "circle"
            for rowInd in range(campaign.X.shape[0])
        ]
    else:
    ...
    for i, param in enumerate(X.columns):
        ...
        fig.update_xaxes(
            tickvals=np.around(np.linspace(0, campaign.m_exp, 5)), row=row_i, col=col_i
        )
    ...
    fig.update_layout(
        ...
        template="ggplot2",
        font_size=fontsize,
        title_text="Campaign Data Visualization",
    )
...
def MDS_plot(campaign: Campaign) -> Figure:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            customdata=campaign.data[list(campaign.X_space.X_names) + ["Iteration"]],
            showlegend=False,
        )
    )
    ...
    fig.update_traces(
        hovertemplate="".join(template)
        + "<b>Iteration</b>"
        + ": %{customdata["
        + str(len(campaign.X_space))
        + "]}<br>"
        + "<b>MDS C1</b>"
        + ": %{x:.3G}<br>"
        + "<b>MDS C2</b>"
...
def parity_plot(
    optimizer: Optimizer, f_transform: bool = False, response_id: int = 0
) -> Figure:
    """
    ...
    fig.update_traces(
        hovertemplate="(%{x:.3G}, %{y:.3G}) +%{error_y.array:.3G}/-%{error_y.arrayminus:.3G}"
    )
    ...
    fig.update_layout(
        template="ggplot2", title="Parity Plot", autosize=False, height=400, width=500
    )
...
def factor_plot(
    ...
    unit_span = np.linspace(
        0, 1 - 1e-15, n_samples
    )  # Prevent issues with cat[1.0] = undefined
    ...
    if objective:
        ...
        if plotRef:
            ...
            Y_mu_ref = Y_pred_ref[
                y_name + ("_t (pred)" if f_transform else " (pred)")
            ].values

    if plotRef:
        fig.add_trace(
            go.Scatter(
                x=X_ref.iloc[:, feature_id].values,
                y=Y_mu_ref,
                mode="markers",
    ...
    fig.add_trace(
        go.Scatter(
            x=X, y=Y_mu, mode="lines", line={"color": obsidian_colors.teal}, name="Mean"
        ),
    )
...
def surface_plot(
    ...
    if plot_data and f_transform:
        raise UnsupportedError(
            "Plotting data is not supported for transformed responses"
        )

    # Create a dataframe of test samples for plotting
    n_grid = 100
    X_test = pd.concat([optimizer.X_best_f] * (n_grid**2), axis=0).reset_index(
        drop=True
    )
    ...
    fig = go.Figure()
    fig.add_trace(go.Surface(z=Z_mu, x=x0_ax, y=x1_ax, name="Mean", opacity=0.85))
    ...
    fig.update_layout(
        ...
        template="ggplot2",
        title=f"Surface Plot for {X0_name} vs {X1_name}",
    ...
    if plot_data:
        ...
        if len(optimizer.X_space) != 2:
            X_t_train_ex = (
                optimizer.X_t_train.copy().drop(columns=[X0_name, X1_name]).values
            )
            X_t_test_ex = optimizer.X_space.encode(
                X_test.copy().drop(columns=[X0_name, X1_name]).drop_duplicates()
            ).values

            if X_t_train_ex.shape[0] > 1:
                X_t_dist = np.linalg.norm(X_t_train_ex - X_t_test_ex, ord=2, axis=1)
                X_t_dist_scaled = (X_t_dist - X_t_dist.min()) / (
                    X_t_dist.max() - X_t_dist.min()
                )
        ...
        fig.add_trace(
            go.Scatter3d(
                x=optimizer.X_train[X0_name],
                y=optimizer.X_train[X1_name],
                z=Z_train[y_name],
                mode="markers",
...
def optim_progress(
    ...
    X_suggest: pd.DataFrame | None = None,
) -> Figure:
    """
    ...
    if not campaign._is_mo or len(response_ids) < 2:
        ...
        out_exp = out_exp.reset_index(drop=False).rename(
    ...
    if X_suggest is not None:
        ...
        if campaign.objective is None:
            ...
            hovertext = (
                out_names[0]
                + " :%{x:.3G} +%{error_x.array:.2G}/- \
                        %{error_x.arrayminus:.2G}<br>"
                + out_names[1]
                + ": %{y:.3G} \
            ...
            if campaign._is_mo:
                ...
                y_mu = pd.concat(
                    [
                        pd.DataFrame(
                            np.arange(m_data, m_data + m_suggest),
                            columns=["Experiment"],
                        ),
                        y_mu,
                    ],
            ...
            hovertext = (
                out_names[0] + " :%{x:.3G}" + "<br>" + out_names[1] + " :%{y:.3G}"
            )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="markers",
                marker=dict(
                    color=obsidian_colors.accent.pastel_blue,
                    symbol="diamond-open",
                    size=7,
                    line=dict(width=2),
                ),
    ...
    fig.update_layout(
        xaxis_title=out_names[0], yaxis_title=out_names[1], title="Optimization Results"
    )
