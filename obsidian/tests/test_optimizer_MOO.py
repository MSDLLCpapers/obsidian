...
def test_optimizer_suggest(m_batch, fixed_var):
    X_suggest, eval_suggest = optimizer.suggest(
        m_batch=m_batch, fixed_var=fixed_var, acquisition=["NEHVI", "SF"], **test_config
    )
...
acq_names = [acq for acq in test_aqs if isinstance(acq, str)]
...
def test_moo_acquisition_determinism(X_space, Z0, acquisition):
    """Test that all MOO acquisition functions respect fix_random_state flag
