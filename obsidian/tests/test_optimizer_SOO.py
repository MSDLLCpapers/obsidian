...
@pytest.mark.parametrize(
    "surrogate",
    [
        pytest.param("GP", marks=pytest.mark.fast),
        "GPflat",
        "GPprior",
...
def test_fit_nan():
    ...
    for col in Z0_sample.columns:
        # Add seed for robustness of test
        Z0_sample.loc[Z0_sample.sample(frac=0.1, random_state=114514).index, col] = np.nan
...
def test_optimizer_pending():
    ...
    X_suggest, eval_suggest = optimizer.suggest(
        m_batch=1, **test_config, X_pending=X_suggest, eval_pending=eval_suggest
    )
...
def test_optimizer_suggest(m_batch, fixed_var):
    X_suggest, eval_suggest = optimizer.suggest(
        m_batch=m_batch, fixed_var=fixed_var, acquisition=["EI", "SF"], **test_config
    )
...
# Acquisition functions with hyperparameters (for suggest tests)
test_aqs = [
    "NEI",
    "EI",
    {"EI": {"inflate": 0.05}},
...
# Base acquisition function names (for determinism tests)
# Extract both strings and dict keys, then remove duplicates
# Sort to ensure deterministic test collection order for pytest-xdist
acq_names = sorted(set([acq if isinstance(acq, str) else list(acq.keys())[0] for acq in test_aqs]))
...
@pytest.mark.parametrize("acquisition", acq_names)
def test_soo_acquisition_determinism(X_space, acquisition):
    """Test that all acquisition functions respect fix_random_state flag

    All acquisition functions should:
    - Be deterministic when fix_random_state=True (default)
    - Be stochastic when fix_random_state=False
    - Support manual_seed override
    """
    from obsidian.tests.utils import assert_acquisition_determinism

    target = Target(name="Response", f_transform="Standard", aim="max")
    simulator = Simulator(X_space, shifted_parab, eps=0.05, rng=555)

    designer = ExpDesigner(X_space, seed=555)
    X0 = designer.initialize(m_initial=5, method="LHS")
    y0 = simulator.simulate(X0)
    Z0 = pd.concat([X0, y0], axis=1)

    assert_acquisition_determinism(X_space, Z0, target, acquisition, m_batch=2, seed=555)


def test_nonlinear_constraints_determinism():
    """Test that nonlinear constraints respect fix_random_state for initial conditions"""
    from obsidian.constraints import BatchVariance_Constraint
    from obsidian.tests.param_configs import X_sp_cont_small

    # Use continuous-only parameter space (nonlinear constraints don't support discrete features)
    X_space = X_sp_cont_small
    target = Target(name="Response", f_transform="Standard", aim="max")
    simulator = Simulator(X_space, shifted_parab, eps=0.05, rng=777)
    ...
    nl_const = BatchVariance_Constraint(X_space, ind=0, tol=0.1)

    # Test with fix_random_state=True (deterministic)
    opt_fixed = BayesianOptimizer(X_space, seed=777, fix_random_state=True)
    opt_fixed.fit(Z0, target)

    X_nl1, _ = opt_fixed.suggest(m_batch=1, acquisition=["EI"], nleq_constraints=nl_const, **test_config)
    X_nl2, _ = opt_fixed.suggest(m_batch=1, acquisition=["EI"], nleq_constraints=nl_const, **test_config)
    pd.testing.assert_frame_equal(X_nl1, X_nl2)

    # Test with fix_random_state=False (stochastic)
    opt_not_fixed = BayesianOptimizer(X_space, seed=777, fix_random_state=False)
    opt_not_fixed.fit(Z0, target)

    X_nl3, _ = opt_not_fixed.suggest(m_batch=2, acquisition=["EI"], nleq_constraints=nl_const, **test_config)
    X_nl4, _ = opt_not_fixed.suggest(m_batch=2, acquisition=["EI"], nleq_constraints=nl_const, **test_config)
    assert not X_nl3.equals(X_nl4), "Nonlinear constraints with fix_random_state=False should give different results"


def test_optimizer_rng_save_load():
    ...
    # Verify loaded optimizer has correct RNG state
    assert optimizer2.seed == 777
    assert hasattr(optimizer2, "rng")
    assert optimizer2.rng.seed == 777
    assert optimizer2.fix_random_state is True
...
@pytest.mark.fast
def test_rejects_bad_manual_seed():
    """fit/suggest/evaluate reject an out-of-range or non-integer manual_seed at the API boundary."""
    X_space = X_sp_default
    target = Target(name="Response", f_transform="Standard", aim="max")
    designer = ExpDesigner(X_space, seed=1)
    X0 = designer.initialize(m_initial=len(X_space) * 2, method="LHS")
    y0 = Simulator(X_space, shifted_parab, eps=0.05, rng=1).simulate(X0)
    Z = pd.concat([X0, y0], axis=1)

    optimizer = BayesianOptimizer(X_space, surrogate="GP", seed=0, verbose=0)

    # bad seed rejected at fit (before the model is even fit)
    with pytest.raises(ValueError, match="manual_seed"):
        optimizer.fit(Z, target=target, manual_seed=-1)

    optimizer.fit(Z, target=target)

    # out-of-range rejected at suggest and evaluate
    with pytest.raises(ValueError, match="manual_seed"):
        optimizer.suggest(m_batch=1, acquisition=["EI"], manual_seed=2**32)
    with pytest.raises(ValueError, match="manual_seed"):
        optimizer.evaluate(X0.iloc[:1], target=target, manual_seed=2**40)

    # non-integer rejected
    with pytest.raises(TypeError, match="manual_seed"):
        optimizer.suggest(m_batch=1, acquisition=["EI"], manual_seed=3.5)

    # valid seed still works
    optimizer.suggest(m_batch=1, acquisition=["EI"], manual_seed=123)
