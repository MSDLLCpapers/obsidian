...
class ParserContext(TypedDict):
    ...
    Attributes:
        ...
        n_obs: Number of training observations (data-state fingerprint). Used by
            randomized acquisition functions to derive a per-iteration random seed that
            is deterministic in the current data, so the random draw varies across
            iterations but is idempotent within a single suggest() call.
    """
...
def default_hyperparameter_parser(
    ...
    Args:
        aq_kwargs (dict[str, Any]): Acquisition function keyword arguments partially processed from the optimizer,
            including all default arguments of the acquisition function
...
class AcquisitionRegistry:
    ...
    def _initialize(self, BUILTIN_CONFIGS: dict[str, dict[str, Any]]):
        ...
        self.aq_defaults: dict[str, dict[str, str]] = {
            ...
            # Placeholder for future characterization acquisition functions
            "characterization": {"single": "RANDSTR", "multi": "JAREX"},
        }
        self.universal_opt_aqs: set[str] = set()
        self.universal_aqs: set[str] = set()
        self.unconstrainable_aqs: set[str] = set()
        # Register built-in functions
        self._register_builtin_functions(BUILTIN_CONFIGS)

    def reset(self, BUILTIN_CONFIGS: dict[str, dict[str, Any]]):
        """Reset the registry to default state with only built-in functions"""
        self._initialize(BUILTIN_CONFIGS)
    ...
    def _register_individual_function(
        ...
        if not config.output_constraints:
            self.unconstrainable_aqs.add(name)
    ...
    def _register_builtin_functions(self, builtin_configs: dict[str, dict[str, Any]]):
        ...
        self.universal_aqs = self.universal_opt_aqs & self.universal_charact_aqs
    ...
    def register_acquisition_function(
        ...
        Args:
            ...
            hp_defaults (dict): Optional hyperparameter defaults.
                If None and implementation provided, attempt to infer.
            ...
            output_constraints (bool): Whether the acquisition function honors output constraints.
                Defaults to True. Set False for acquisitions that don't accept/forward
        ...
        # Default to the default parser, override only if needed
        hyperparameter_parser = default_hyperparameter_parser

        # Case 1: Reuse internal parser from existing config (when overloading)
        if reuse_parser:
            if not exists:
                raise ValueError(
                    "reuse_parser=True requested but acquisition function does not"
                    "already exist to take the internal parser from."
                )
            hyperparameter_parser = self.configs[name].hyperparameter_parser

        # Case 2: Use externally provided parser
        if parser is not None:
            if reuse_parser:
                raise ValueError("Cannot supply both parser and reuse_parser=True.")
    ...
    @property
    def valid_opt_aqs(self) -> dict[str, set[str]]:
        """Backward compatibility for valid_aqs"""
        return self.valid_aqs["optimization"]

    @property
    def valid_charact_aqs(self) -> dict[str, set[str]]:
        """Backward compatibility for valid_aqs"""
...
def _nipv_hyperparameter_parser(
    ...
    Uses a fixed seed for Sobol QMC sampling to ensure stable integration points
    across multiple suggestions. This improves stability and reproducibility.
    """
    n_dim = context["n_dim"]
    ...
    # Use fixed seed for Sobol QMC points
    # This ensures stable numerical integration across suggestions
    seed = hps.get("seed", None) or 114514
    n_mc_points = hps.get("n_mc_points", 128)
    X_bounds = torch.tensor([[0.0, 1.0]] * n_dim, dtype=TORCH_DTYPE).T
    qmc_samples = draw_sobol_samples(bounds=X_bounds, n=n_mc_points, q=m_batch, seed=seed)
