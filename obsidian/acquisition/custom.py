...
class RandomSampling(torch.nn.Module):
    def __init__(
        ...
        if generator is None:
            # Create fresh generator that inherits seed from Python's random module.
            # Behavior depends on context:
            # - New RNG control + fix_random_state=True: deterministic (same seed each call)
            # - New RNG control + fix_random_state=False: stochastic (different seed each call)
            # - New RNG control + manual_seed override: uses that specific seed
            # - Old RNG control: deterministic (same seed each call)
            # - Standalone (outside suggest): truly random or up to user pass-in generator
            from obsidian.rng import get_new_seed

            seed: int = get_new_seed(1)  # type: ignore
